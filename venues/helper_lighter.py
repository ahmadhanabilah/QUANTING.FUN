...
class LighterWS:
    ...
    def __init__(self, symbol: str, read_only: bool = False):
        ...
        self.config = {
            ...
            "account_id": os.getenv("LIGHTER_ACCOUNT_ID"),
        }
    ...
    async def _init_market_id(self) -> None:
        ...
        logger.info(f"[LighterWS] {self.symbol} → market_id={self.market_id}")

    async def load_initial_position(self) -> tuple[float, float]:
        """Load current position via REST."""
        ...
        try:
            ...
            logger.error(f"[LighterWS:{self.symbol}] load_initial_position error: {exc}")
            return 0.0, 0.0
    ...
    async def _init_trading_client(self) -> None:
        ...
        self.trading_client = lighter.SignerClient(
            ...
            api_key_index=int(self.config["api_key_index"]),
        )
    ...
    async def _ensure_trade_ws(self) -> None:
        ...
        # consume connected message if any
        try:
            msg = await self._ws_trade.receive(timeout=1)
            logger.debug(f"[LighterWS:{self.symbol}] trade ws connected msg={msg}")
        except Exception:
            pass
        ...
        async def run_ws():
            while True:
                try:
                    ...
                    logger.info(f"[LighterWS:{self.symbol}] subscribing order_book_ids={self.market_id}")
                    await self.ws_client.run_async()
                except Exception as e:
                    logger.error(f"[LighterWS:{self.symbol}] error: {e}; reconnecting in 1s")
    ...
    def _handle_orderbook(self, market_id: int, order_book: dict) -> None:
        try:
            ...
            logger.error(f"[LighterWS:{self.symbol}] handle_orderbook error: {e}")
        ...
        try:
            ...
            for t in trades:
                try:
                    ...
                    logger.debug(f"[LighterWS:{self.symbol}] trade parse error: {exc}")
                    continue
            ...
            logger.error(f"[LighterWS:{self.symbol}] handle_account_update error: {exc}")
            ...
            while True:
                try:
                    ...
                    async with self._ws_session.ws_connect(url, heartbeat=30) as ws:
                        ...
                        logger.info(f"[LighterWS] Subscribed to account_all_trades/{self.config['account_id']}")
                        async for msg in ws:
                            if msg.type == aiohttp.WSMsgType.TEXT:
                                ...
                                if isinstance(data, dict) and data.get("error", {}).get("code") == 20013:
                                    logger.warning("[LighterWS] auth expired; refreshing token")
                                    await self._refresh_auth_token(force=True)
                                    break  # reconnect with new token
                                self._handle_account_update(data)
                            elif msg.type == aiohttp.WSMsgType.ERROR:
                                raise RuntimeError(f"Lighter account WS error: {ws.exception()}")
                    logger.warning("[LighterWS] account WS closed; reconnecting in 1s")
                    await asyncio.sleep(1)
                except Exception as exc:
                    logger.error(f"[LighterWS] account WS error: {exc}; reconnecting in 1s")
                    await asyncio.sleep(1)
    ...
    async def place_limit(self, side: Side, price: float, size: float) -> Optional[int]:
        ...
        try:
            ...
            logger.info(
                f"[LighterWS] place_limit payload market_id={self.market_id} "
            ...
            logger.info(f"[LighterWS] limit order {client_order_index} placed ({latency_ms:.2f} ms)")
            return client_order_index
        except Exception as exc:
            logger.error(f"[LighterWS] place_limit failed: {exc}")
            return None
    ...
    async def cancel(self, order_id: int) -> None:
        ...
        try:
            ...
            logger.info(f"[LighterWS] cancelled order {order_id} via WS")
        except Exception as exc:
            logger.error(f"[LighterWS:{self.symbol}] cancel failed: {exc}")

    async def send_market(self, side: Side, size: float, use_ws: bool = True) -> Optional[dict]:
        """Market order via WS (default) or REST."""
        if use_ws:
            return await self._send_market_ws(side, size)
        return await self._send_market_rest(side, size)

    async def _send_market_ws(self, side: Side, size: float) -> Optional[dict]:
        if not self.trading_client or self.market_id is None:
        ...
        logger.info(
            f"[LighterWS] send_market payload "
            f"market_id={self.market_id} price={px} size={base_amount} is_ask={is_ask}"
        )
        try:
            api_start = time.perf_counter()
            await self._ensure_trade_ws()
            tx_type, tx_info, err = self.trading_client.sign_create_order(
                market_index=self.market_id,
                client_order_index=client_order_index,
                base_amount=base_amount,
                price=px,
            ...
            logger.info(
                f"[LighterWS] send_market done order_id={client_order_index} api_ms={api_ms:.1f} total_ms={total_ms:.1f} "
                f"price={px} size={base_amount} is_ask={is_ask}"
            )
            sent_qty = float(base_amount) / (10 ** (self.size_decimals or 0))
            return {"filled_qty": sent_qty}
        except Exception as exc:
            logger.error(f"[LighterWS:{self.symbol}] market order failed: {exc}")
            return None

    async def _send_market_rest(self, side: Side, size: float) -> Optional[dict]:
        if not self.trading_client or self.market_id is None:
            logger.error("Lighter trading client not initialized")
            return None
        if not self.ob["bidPrice"] or not self.ob["askPrice"]:
        ...
        try:
            ...
            logger.error(f"[LighterWS:{self.symbol}] REST market order failed: {exc}")
            return None
    ...
    async def _send_tx(self, tx_type, tx_info) -> None:
        ...
        try:
            msg = await self._ws_trade.receive(timeout=1)
            logger.debug(f"[LighterWS:{self.symbol}] send_tx resp={msg}")
        except Exception:
            pass
