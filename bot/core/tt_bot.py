...
class TTBot:
    ...
    async def loop(self):
        ...
        async with self._exec_lock:
            if isinstance(decision, tuple):
                ...
                for d in decision:
                    await self._execute_single(d, log_decision=True)
            else:
    ...
    def _log_test_trade_if_needed(self, decision: Decision):
        ...
        asyncio.create_task(self._push_test_trade_db(ctx_snapshot))
        ...
        self._log_trade_complete(ctx_snapshot=ctx_snapshot)
        logger_maker.info("[TEST TRADE LOGGED] simulated fills recorded (no fills/latencies)")

    def _log_trade_complete(self, ctx_snapshot: dict | None = None):
        ...
        if trace_val:
            try:
                ...
                logger_dec_db.exception("decision_db_push_failed")
    ...
    async def _push_trade_db(self, trace: str, ts: float, venue: str, size: float,
        ...
        try:
            ...
            logger_dec_db.exception("trade_db_push_failed")
    ...
    async def _push_fill_db(self, trace: str, ts: float, venue: str,
        ...
        try:
            ...
            logger_dec_db.exception("fill_db_push_failed")
    ...
    async def _send_market(self, d: Decision, log_decision: bool = True):
        ...
        if not result:
            ...
            asyncio.create_task(
                self._push_trade_db(
                    trace=getattr(d, "_trace", None)
                    or getattr(self, "_current_trace", None)
