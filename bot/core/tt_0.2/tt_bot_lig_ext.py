...
class TTBot:
    ...
    async def _log_decision_db(self, initial: bool, inv_after=None, fill_info=None, ctx=None):
        ...
        ob_str_L = f"{obL.get('bidPrice')}:{obL.get('bidSize')}/{obL.get('askPrice')}:{obL.get('askSize')}"
        ob_str_E = f"{obE.get('bidPrice')}:{obE.get('bidSize')}/{obE.get('askPrice')}:{obE.get('askSize')}"
