...
class TTBot:
    def __init__(self, state, venue1, venue2,
                 symVenue1, symVenue2,
                 minSpread, spreadTP,
        ...
        self.state              = state
        # retain legacy aliases for internal logic
        self.V1                 = venue1
        self.V2                 = venue2
        self.L                  = venue1
        self.E                  = venue2
        self.symbolL            = symVenue1
        self.symbolE            = symVenue2
...
async def run_tt_bot(symbol_l: str | None = None, symbol_e: str | None = None, config: dict | None = None):
    ...
    venue1_type = str(cfg_item.get("VENUE1", "LIGHTER")).upper()
    venue2_type = str(cfg_item.get("VENUE2", "EXTENDED")).upper()
    ...
    # pick venues based on config (currently LIGHTER/EXTENDED only)
    def _build_venue(vtype: str, sym: str):
        vt = vtype.upper()
        if vt.startswith("LIGHT"):
            return LighterWS(sym)
        if vt.startswith("EXT"):
            return ExtendedWS(sym)
        raise RuntimeError(f"Unsupported venue type {vtype}")

    # load per-venue envs if present
    env_dir = PROJECT_ROOT / "env"
    env_dir.mkdir(exist_ok=True)
    for vt in {venue1_type, venue2_type}:
        env_path = env_dir / f".env_{vt.lower()}"
        if env_path.exists():
            _load_env_files([env_path])

    V1 = _build_venue(venue1_type, symbolL)
    V2 = _build_venue(venue2_type, symbolE)
    V1.dedup_ob = state.dedup_ob
    V2.dedup_ob = state.dedup_ob
    state.hedge_seeded = True

    # hydrate venue metadata before logging config so min sizes/values are populated
    try:
        await V1._init_market_id()
    except Exception:
        pass
    try:
        await V2._load_market_info()
    except Exception:
        pass
    ...
    logging.getLogger("_TT").info(
        "[CONFIG] VENUE1=%s(%s) VENUE2=%s(%s) minSpread=%s spreadTP=%s "
    ...
    async def maker_loop():
        def on_update():
            ...
            if V1.ob["bidPrice"] and V1.ob["askPrice"] and V2.ob["bidPrice"] and V2.ob["askPrice"]:
                spreads = calc_spreads(V1, V2, state)
                snap = (
                    V1.ob["bidPrice"],
                    V1.ob["bidSize"],
                    V1.ob["askPrice"],
        ...
        V1.set_ob_callback(on_update)
        V2.set_ob_callback(on_update)
        ...
        V1.set_inventory_callback(on_inv_l)
        V2.set_inventory_callback(on_inv_e)
        V1.set_position_state_callback(lambda qty, entry: maker_bot._on_position_update("L", qty, entry))
        V2.set_position_state_callback(lambda qty, entry: maker_bot._on_position_update("E", qty, entry))
        ...
        l_qty, l_entry = await V1.load_initial_position()
        e_qty, e_entry = await V2.load_initial_position()
        state.invL = l_qty
