...
Usage:
  /root/arb_bot/.venv/bin/python -m core.latency_tester BTC BTC-USD --size 0.0003

Requires trading credentials in .env (EXTENDED_*, LIGHTER_*).
...
async def run_latency_test(symbol_l: str, symbol_e: str, size: Optional[float] = None) -> int:
    ...
    try:
        ...
        for i, (ext_side, light_side) in enumerate(zip(extended_seq, lighter_seq), start=1):
            ...
            res_e = await extended.send_market(ext_side, chosen_e)
            ...
            res_l = await lighter.send_market(light_side, chosen_l)
